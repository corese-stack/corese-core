package fr.inria.corese.core.next.data.impl.io.parser.common;
...
public abstract class AbstractNTriplesNQuadsListener {
    ...
    public String stripAngles(String iriRef) {
        if (iriRef.length() < 2) {
            throw new ParsingErrorException("Invalid IRI reference: too short");
        }
    ...
    public String extractBlankNodeLabel(String text) {
        if (text.length() < 2) {
            throw new ParsingErrorException("Invalid blank node: too short");
        }
    ...
    public String removeQuotes(String text) {
        ...
        throw new ParsingErrorException("Literal does not have expected quotes: " + text);
    }
