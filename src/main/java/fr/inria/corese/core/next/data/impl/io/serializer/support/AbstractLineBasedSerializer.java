...
public abstract class AbstractLineBasedSerializer implements RDFSerializer {
    ...
    protected void writeLiteral(Writer writer, Literal literal) throws IOException {
        ...
        if (!literal.getLanguage().isPresent() && datatype != null &&
                (config.getLiteralDatatypePolicy() == LiteralDatatypePolicy.ALWAYS_TYPED ||
                        (config.getLiteralDatatypePolicy() == LiteralDatatypePolicy.MINIMAL && !datatype.equals(XSDDatatype.STRING.getIRI())))) {
    ...
    protected void validateLiteral(Literal literal) {
        ...
        if (literal.getLanguage().isPresent()) {
            if (datatype == null || !datatype.stringValue().equals(RDFDatatype.LANGSTRING.getIRI().stringValue())) {
