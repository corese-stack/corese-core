...
import static fr.inria.corese.core.next.query.kgram.sparql.datatype.CoreseBoolean.FALSE;
import static fr.inria.corese.core.next.query.kgram.sparql.datatype.CoreseBoolean.TRUE;
import static fr.inria.corese.core.next.query.kgram.sparql.datatype.Cst.jTypeGenericInteger;
import static fr.inria.corese.core.next.query.kgram.sparql.datatype.Cst.jTypeInteger;
 ...
 */
public abstract class CoreseDatatype
        implements IDatatype {
    static final CoreseURI datatype = new CoreseURI(fr.inria.corese.core.next.data.impl.common.vocabulary.RDFS.Resource.getIRI().stringValue());
    ...
    static Datatype code = Datatype.UNDEFINED;

    static HashMap<Datatype, Class> javaClassMap;
    @Deprecated
    ...
    static void init() {
        ...
        define(Datatype.INTEGER, int.class);
        define(Datatype.DOUBLE, double.class);
        define(Datatype.DECIMAL, double.class);
        define(Datatype.FLOAT, double.class);
        define(Datatype.BOOLEAN, boolean.class);
        define(Datatype.STRING, String.class);
        define(Datatype.LITERAL, String.class);
        define(Datatype.URI, String.class);
    }
    ...
    static void define(Datatype i, Class c) {
    ...
    public static IDatatype create(String valueJType,
        ...
        try {
            ...
            if ((CoreseDatatype.isNumber(datatype, valueJType) ||
                    datatype.equals(XSD.xsdDate.getIRI().stringValue()))
    ...
    public static IDatatype create(String valueJType,
        ...
        switch (valueJType) {
            case Cst.jTypeString:
                return new CoreseString(label);
            case Cst.jTypeURI:
                return new CoreseURI(label);
            case Cst.jTypeLiteral:
                return new CoreseLiteral(label, lang);
        }
    ...
    public String toSparql(boolean prefixDatatype, boolean displayDatatype, boolean skipUndefPrefix, NSManager nsm) {
        ...
        if (getCode() == Datatype.INTEGER && !displayDatatype && getDatatypeURI().equals(XSD.xsdInteger.getIRI().stringValue())
                && (!(value.startsWith("0") && value.length() > 1))) {
            // display integer value as is (without datatype)
        } else if (getCode() == Datatype.BOOLEAN && !displayDatatype &&
                (getLabel().equals(CoreseBoolean.STRUE) || getLabel().equals(CoreseBoolean.SFALSE))) {
        } else if (getCode() == Datatype.STRING || (getCode() == Datatype.LITERAL && !hasLang())) {
            value = protect(value);
        } else if (getDatatype() != null && !getDatatype().getLabel().equals(RDF.langString.getIRI().stringValue())) {
            String datatype = getDatatype().getLabel();

            if (prefixDatatype && (datatype.startsWith(XSD.getVocabularyNamespace()))
                    || datatype.startsWith(RDF.getVocabularyNamespace())
    ...
    Iterator<IDatatype> loopIterator() {
        return new Iterator<>() {
            final Iterator<IDatatype> it = getLoop().iterator();
    ...
    public IDatatype isWellFormed() {
        if (isLiteral() && isUndefined()) {
            if (getDatatypeURI().startsWith(XSD.getVocabularyNamespace())) {
                return FALSE;
            } else if (getDatatypeURI().startsWith(RDF.getVocabularyNamespace()) && !getDatatypeURI().equals(RDF.HTML.getIRI().stringValue())) {
                return FALSE;
            }
        }
        return TRUE;
    ...
    public int compareTriple(IDatatype dt) throws CoreseDatatypeException {
        throw new CoreseDatatypeException("Triple comparison is not implemented");
    }

    /**
     * @TODO Refactor/re-code this comparison function. A lot of those cases should be delegated to upper or lower classes
     */
    @Override
    public int compareTo(IDatatype d2) {
        Datatype code = getCode(), other = d2.getCode();
        switch (other) {
            case STRING:
                if (code == other) {
                ...
                if (other == Datatype.LITERAL) {
                    ...
                    if (res == 0) {
                        if (d2.hasLang()) {
                            ...
                            return getCode().compareTo(d2.getCode());
                        }
                    }
                    return res;
                }
    ...
    public int compare(IDatatype iod) throws CoreseDatatypeException {
        throw new CoreseDatatypeException("compare(IDatatype) not implemented");
    }

    @Override
    public boolean less(IDatatype iod) throws CoreseDatatypeException {
        throw new CoreseDatatypeException("less not implemented");
    }

    @Override
    public boolean lessOrEqual(IDatatype iod) throws CoreseDatatypeException {
        throw new CoreseDatatypeException("lessOrEqual not implemented");
    }

    @Override
    public boolean greater(IDatatype iod) throws CoreseDatatypeException {
        throw new CoreseDatatypeException("greater not implemented");
    }

    @Override
    public boolean greaterOrEqual(IDatatype iod) throws CoreseDatatypeException {
        throw new CoreseDatatypeException("greaterOrEqual not implemented");
    }
    ...
    public boolean equalsWE(IDatatype iod) throws CoreseDatatypeException {
        throw new CoreseDatatypeException("equalsWE not implemented");
    }
