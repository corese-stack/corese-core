...
public class SparqlParser extends AbstractQueryParser implements QueryTextValidator {
    ...
    public SparqlParser(SparqlParserOptions config) {
        super(config);
    }
    @Override
    public QueryAst parse(InputStream in) {
        return parse(new java.io.InputStreamReader(in, StandardCharsets.UTF_8), getConfig().getBaseIRI());
    }
    ...
    public QueryValidationResult validate(InputStream in) {
        return validate(new java.io.InputStreamReader(in, StandardCharsets.UTF_8), getConfig().getBaseIRI());
    }
