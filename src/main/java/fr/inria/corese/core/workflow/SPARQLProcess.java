...
public class SPARQLProcess extends WorkflowProcess {
    ...
    String tuneQuery(QueryProcess exec, Data data) {
        ...
        return tune(exec, data, getQuery());
    }

    /**
     * Replace $pattern $mode $param by pattern() st:get(st:mode) st:get(st:param)
     */
    String tune(QueryProcess exec, Data data, String query) {
        query = pattern(exec, data, query);
     ...
     */
    String pattern(QueryProcess exec, Data data, String query) {
        IDatatype name = getValue(getProcess(), Context.STL_PROCESS_QUERY);
        if (name == null) {
            return query;
        }
        try {
            IDatatype res = exec.funcall(name.stringValue(), getContext(), data.getBinding(),
                    DatatypeMap.newInstance(query));
            if (res != null) {
