...
import fr.inria.corese.core.next.kgram.core.ProcessVisitorDefault;
...
public class Extension extends Core {
    ...
    public IDatatype list(IDatatype dt) {
        if (dt.isExtension()) {
            ...
            if (IDatatype.MAPPINGS_DATATYPE.equals(datatypeURI)) {
                return list(dt.getPointerObject().getMappings());
            } else if (IDatatype.MAPPING_DATATYPE.equals(datatypeURI)) {
                return list(dt.getPointerObject().getMapping());
            }
        }
