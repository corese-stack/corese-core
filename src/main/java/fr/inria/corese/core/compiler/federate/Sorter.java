...
public class Sorter {
    ...
    boolean hasFilter(Exp exp, SorterResult res) {
        if (exp.isFilter()) {
            ...
            if ((hasFilter(exp.get(0)) && (hasFilter(exp.get(1)) || hasConstant(exp.get(1)))) || (hasConstant(exp.get(0)) && hasFilter(exp.get(1)))) {
                res.incrFilter();
            }
        } else if (exp.isOptional() || exp.isMinus()) {
    ...
    boolean connected(List<Variable> l1, List<Variable> l2) {
        for (Variable variable : l1) {
            if (l2.contains(variable)) {
                return true;
            }
        }
        return false;
    ...
    void include(List<Variable> l1, List<Variable> l2) {
        for (Variable variable : l2) {
            if (!l1.contains(variable)) {
                l1.add(variable);
            }
        }
    }
