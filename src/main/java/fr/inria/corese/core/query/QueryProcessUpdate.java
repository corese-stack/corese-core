...
public class QueryProcessUpdate {
    ...
    Mappings overWrite(String name, Query query, Mapping m, Dataset ds, boolean toName) throws EngineException {
        ...
        if (toName) {
            ...
            process = QueryProcess.create(gg);
        } else {
            // eval where bgp on std graph or DataManager
            process = getQueryProcess().copy();
        }
        // bypass lock if any
        Mappings map = process.getQueryProcessUpdate().update(query, m, ds);
        complete(process.getAST(query).getUpdate(), g);
        return map;
    }
    ...
    void complete(ASTUpdate up, Graph g) {
        ...
        for (Update act : up.getUpdates()) {
            if ((act.type() == Update.Keyword.DROP) && (act.getBasic().getGraph() != null && act.getBasic().getGraph().equals(name))) {
