...
class GraphMutationOperationsTest {
    ...
    private CoreseGraphStatementStore store;
    private GraphMutationOperations mutations;

    @BeforeEach
    void setUp() {
        store = mock(CoreseGraphStatementStore.class);
        mutations = new GraphMutationOperations(store);
    }

    @Test
    void rejectsMissingDependenciesAndArguments() {
        assertThrows(IllegalArgumentException.class, () -> new GraphMutationOperations(null));
