...
class SparqlParserOptionsTest {
    ...
    void getDiagnosticsReturnsUnmodifiableList() {
        SparqlParserOptions opts = new SparqlParserOptions.Builder()
                .collectErrors(true)
                .build();
        opts.addDiagnostic(new QueryDiagnostic(
                QueryDiagnostic.Kind.SYNTAX_ERROR,
                QueryDiagnostic.Severity.ERROR,
                "err1", 1, 0, null, null));

        List<QueryDiagnostic> diagnostics = opts.getDiagnostics();
    ...
    void addDiagnosticWhenCollectErrorsTrueAddsAndGetErrorsFormatsThem() {
        ...
        assertEquals(QueryDiagnostic.Kind.LEXER_ERROR, opts.getDiagnostics().get(0).kind());
        assertEquals(1, opts.getDiagnostics().get(0).line());
        assertEquals(QueryDiagnostic.Kind.SYNTAX_ERROR, opts.getDiagnostics().get(1).kind());
